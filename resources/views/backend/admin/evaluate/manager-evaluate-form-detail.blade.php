                        ...
                        <div class="card-body table-responsive text-nowrap">
                            <table class="table table-responsive table-bordered table-hover">
                                ...
                                @foreach ($rates as $rate => $value)
                                    @php
                                        $comment = DB::table('manager_rates')
                                            ->where('id', $value->id)
                                            ->value('comment');
                                    @endphp
                                    <tbody>
                                        <tr>
                                            @php
                                                $scoreValue = DB::table('evaluation_managers')
                                                    ->where('id', $value->evaluation_id)
                                                    ->value('score');
                                                $score = $scoreValue / 5;
                                                $score1 = $scoreValue - 0;
                                                $score2 = $score1 - $score;
                                            ...
                                            <td>
                                                <?php
                                                $number = DB::table('evaluation_managers')->where('id', $value->evaluation_id)->value('number');
                                                $list = DB::table('evaluation_managers')->where('id', $value->evaluation_id)->value('list');
                                                ?>
                                                {{ $number }} {{ $list }}
                                            </td>
                                            <td>
