...
class AdminController extends Controller
{
    ...
    public function showDataEmployeeResignByBranch(Request $request, $branch_id) {
        $NUM_PAGE = 20;
        $employees = Employee::where('branch_id',$branch_id)->where('status','พ้นสภาพพนักงาน')->paginate($NUM_PAGE);
        $page = $request->input('page');
        $page = ($page != null)?$page:1;
    ...
    public function elearningStatistic(Request $request, $branch_id){
        $NUM_PAGE = 20;
        $statistics = ElearningStatistic::paginate($NUM_PAGE);
        $page = $request->input('page');
        $page = ($page != null)?$page:1;
        return view('backend/admin/employee/elearning-statistic')->with('NUM_PAGE',$NUM_PAGE)
                                                                 ->with('page',$page)
    ...
    public function listEmployeeEvaluateByYear(Request $request, $branch_id, $date_y) {
        $NUM_PAGE = 50;
        $employees = Employee::join('positions', 'employees.position_id', '=', 'positions.id')
                             ->select('employees.*', 'positions.position')   
                             ->where('branch_id',$branch_id)
